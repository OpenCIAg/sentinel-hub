// var geojson = { "type": "FeatureCollection", "features": [{ "type": "Feature", "properties": {}, "geometry": { "type": "Polygon", "coordinates": [[[-54.2525053024292, -15.317445315092336], [-54.252848625183105, -15.315934543109869], [-54.24018859863281, -15.309849953728827], [-54.23975944519043, -15.311340073609022], [-54.2525053024292, -15.317445315092336]]] } }] }
var map
var drawnItems
var layers = []
...
var loadingGif = "https://i.stack.imgur.com/h6viz.gif"
var filter = "TRUE_COLOR"
var dataBusca = new Date()

async function CreateMap() {
    map = L.map('mapid')
    L.control.layers({
        'Satélite': L.mapbox.tileLayer('bobbysud.map-l4i2m7nd', {
            detectRetina: true
        }).addTo(map),
        'Terreno': L.mapbox.tileLayer('bobbysud.i2pfp2lb', {
            detectRetina: true
        })
    }).addTo(map);
    ...
    document.getElementById('filtros').onchange = function (e) {
        filter = e.target.value
        buscaData()
    }

    map.setView(await getLocation(), 13);
...
async function RefreshPolys(geojson) {
    clearMapLayers()
    loading(geojson)
    const images = await getImageAndPolys(geojson)
    images.forEach(element => {
        layers.push(L.imageOverlay(element.img, element.LatLng).addTo(map))
    });

}
function loading(geojson) {
    SentinelHubWms.latLngToXYTool(geojson).map((LG) => {
        layers.push(L.imageOverlay(loadingGif, LG.getBobxConnors()).addTo(map))
    })
}
...
async function buscaData() {
    clearImgemsData()
    de = document.getElementById('data_busca_de').value
    para = document.getElementById('data_busca_ate').value
    moment(de).diff(moment(para), 'months', true)
    diffMonth = moment(de).diff(moment(para), 'months', true)
    arr = Array.from({ length: diffMonth+1 }, (v, k) => k )
    drawnItems.toGeoJSON().features.forEach(async (data) => {
        arr.forEach((month) =>{
            img = document.createElement("img")
            debugger
            img.setAttribute("id",month)
            img.style.width = "100"
            img.style.height = "100"
            img.src = loadingGif
            document.getElementById("imagens_data").appendChild(img)
            getImageAndPoly(data, new Date(moment(de).subtract(month, 'M').format('MM-DD-YYYY'))).then(polly =>{
                img = document.getElementById(month)
                img.src = polly.img
                img.LatLng = polly.LatLng
            })
            
            img.onclick = (e) =>{
                debugger
                addLayer(e.target.src,e.target.LatLng)
            }
        })
    })
    dataBusca = new Date(document.getElementById('data_busca_ate').value)
    RefreshPolys(drawnItems.toGeoJSON())
}
function addLayer(img,LatLng){
    layers.push(L.imageOverlay(img, LatLng).addTo(map))
}
function clearImgemsData() {
    document.querySelector("#imagens_data").innerHTML = ""
}
async function getImageAndPoly(geojsonFeature, custonData) {
    return await SentinelHubWms.geoJsonToShapeImg(geojsonFeature, "0e01ffdd-d856-4479-a32f-e25fdeaa0eb9", { layers: filter, date: custonData ? custonData : dataBusca })
}
async function getImageAndPolys(geojson, custonData) {
    return await SentinelHubWms.geoJsonToShapeImgs(geojson, "0e01ffdd-d856-4479-a32f-e25fdeaa0eb9", { layers: filter, date: custonData ? custonData : dataBusca })
}
async function newShape(geoJsonFeature) {
    loading(geoJsonFeature)
    const img = await getImageAndPoly(geoJsonFeature)
    addLayer(img.img,img.LatLng)
}
