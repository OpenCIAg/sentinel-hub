...
async function RefreshPolys(geojson) {
    clearMapLayers()
    loading(geojson)
    const images = await getImagesAndPolys(geojson)
...
async function buscaData() {
    ...
    for (let i = 0; i < diffMonth+1; i++) {
        ...
        img.src = (await getImagesAndPolys(featureGroup.toGeoJSON(),new Date(moment(de).subtract(i ,'M').format('MM-DD-YYYY'))))[0].img

    }
    dataBusca = new Date(document.getElementById('data_busca_ate').value)
    RefreshPolys(featureGroup.toGeoJSON())
}
function clearImgemsData(){
    document.querySelector("#imagens_data").innerHTML = ""
}
async function getImagesAndPolys(geojson,custonData){
    PolygonRestrains = SentinelHubWms.latLngToXYTool(geojson)
    console.log(custonData?custonData:dataBusca)
    return  await SentinelHubWms.geoJsonToShapeImg(geojson, "0e01ffdd-d856-4479-a32f-e25fdeaa0eb9", { layers: filter, date: custonData?custonData:dataBusca })
}
